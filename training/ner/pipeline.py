...
def ner_pipeline(model, device, experiment_name, checkpoint_name=None, is_bert=False):
    epochs = 100
    batch_size = 128
    ...
    if is_bert:
        ...
        val_postprocess_batch_fn = None
    output_dir = Path(os.getenv('SCRATCH_DIR')) / 'models' / model.name / 'rurebus' / experiment_name
    ...
    losses = [CrossEntropyLoss(# weights=1 / bilou_counts[ent_type] ** 0.1,
                               label_smoothing=0.01).to(device)
              for ent_type in rurebus.entity_types]
    # losses = [LossList([CrossEntropyLoss(label_smoothing=0.01),
    #                     DiceLoss(weights=[0, 1, 1, 1, 1])],
    #                    ratios=[0.9, 0.1]).to(device)
    #           for ent_type in rurebus.entity_types]
    ...
    if not is_bert:
        callbacks.append(FreezeEmbeddingsCallback(epochs=20))
